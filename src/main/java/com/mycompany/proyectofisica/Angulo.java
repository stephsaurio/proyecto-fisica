 ...
 * @author gomez
 */
public class Angulo extends javax.swing.JFrame {

    /**
     * Creates new form Angulo
     */
    public Angulo() {
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Calcular el angulo");
        ...
        getContentPane().add(formula, new org.netbeans.lib.awtextra.AbsoluteConstraints(730, 220, 630, 240));
        ...
        unidadSalida.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "°", "rad" }));
        ...
        getContentPane().add(VelocidadInicialField, new org.netbeans.lib.awtextra.AbsoluteConstraints(340, 240, 260, 30));
        ...
        getContentPane().add(unidadVelocidadInicial, new org.netbeans.lib.awtextra.AbsoluteConstraints(600, 240, 110, 30));
    ...
    if (unidadVelocidadInicial.getSelectedItem().equals("km/h")) V0 /= 3.6;
    if (unidadV0x.getSelectedItem().equals("km/h")) V0x /= 3.6;
    if (unidadV0y.getSelectedItem().equals("km/h")) V0y /= 3.6;
    if (unidadAlcance.getSelectedItem().equals("km")) alcance *= 1000;
    if (unidadhMax.getSelectedItem().equals("km")) hmax *= 1000;
    if (unidadTtotal.getSelectedItem().equals("horas")) tiempo *= 3600;
    if (unidadTtotal.getSelectedItem().equals("minutos")) tiempo *= 60;

    double g = 9.81;
    double anguloRad = 0;
    String formulaUsada = "";

    // Orden de prioridad de fórmulas
    if (V0x != 0 && V0y != 0) {
        anguloRad = Math.atan2(V0y, V0x);
        formulaUsada = "θ = atan2(V₀y, V₀x)";
    } else if (V0 != 0 && V0x != 0) {
        anguloRad = Math.acos(V0x / V0);
        formulaUsada = "θ = arccos(V₀x / V₀)";
    } else if (V0 != 0 && V0y != 0) {
        anguloRad = Math.asin(V0y / V0);
        formulaUsada = "θ = arcsen(V₀y / V₀)";
    } else if (V0 != 0 && alcance != 0) {
        double val = (alcance * g) / (V0 * V0);
        if (val >= -1 && val <= 1) {
            anguloRad = 0.5 * Math.asin(val);
            formulaUsada = "θ = ½·arcsen((alcance·g)/V₀²)";
        } else {
            JOptionPane.showMessageDialog(this, "Valor inválido para arcsen: fuera de rango.");
            return;
        }
    } else if (tiempo != 0 && V0 != 0) {
        anguloRad = Math.asin((g * tiempo) / (2 * V0));
        formulaUsada = "θ = arcsen((g·t)/(2·V₀))";
    } else if (hmax != 0 && V0 != 0) {
        double Vy = Math.sqrt(2 * g * hmax);
        anguloRad = Math.asin(Vy / V0);
        formulaUsada = "θ = arcsen(√(2·g·hₘₐₓ)/V₀)";
    } else {
        JOptionPane.showMessageDialog(this, "Ingresa una combinación válida de datos para calcular el ángulo.");
        return;
    }

    double anguloFinal = unidadSalida.getSelectedItem().equals("rad") ?
                         anguloRad :
                         Math.toDegrees(anguloRad);

    Respuesta.setText(String.format("%.2f %s", anguloFinal, unidadSalida.getSelectedItem()));
